...
若未提供八字，则声明“八字信息不全，仅依梅花卦象独断”，跳过此步，后续输出不得虚构八字内容。

### 第三步：八卦万物类象 · 具象映射（必须扣住“用户问题”）
根据用户问的 {question}，强制从以下库中取象，并将抽象五行转化为具体人、事、物：
- **乾/兑（金）**：领导、法律、金融、武职、刚毅、圆形、白色。
- **离（火）**：文书、文化、照明、电子、女性、中女、急躁、红色。
- **震/巽（木）**：长男、经营、快递、文书、草本、车船、绿色。
- **坎（水）**：财险、暗流、欺诈、流动性、男性、中年、黑色。
- **艮/坤（土）**：房产、土地、后勤、长辈、脾胃、黄色、稳定。

结合体用卦的所属五行，直接描绘出当前局面下**关键人物的性格特征**、**所处环境的具体方位**以及**资金/物质的流动方向**。

### 第四步：应期推断（结合动爻与节气）
- 以 **动爻数** 定基础应期单位（爻数 1-6 对应日/周/月，需根据问题急缓自断，例如：问“明天面试”按日推，问“年内财运”按月推）。
- 结合 **变卦的卦气**（如震巽主春、离主夏、乾兑主秋冬）缩小时间窗口。
- **最终输出必须给出一个明确的时间范围**（例如：“3日内（尤指午、未日）”或“农历八月申酉金旺之时”）。
...
- **应期指向**：（例：农历五月午火当令，或本月7日之前）

## 💡 锦囊三策（必须具体到行为，不可写“心态要稳”这类空话）
1. 【急策】：（针对未来3天的具体操作，如“今日申时向正西步行百步，并暂停回复邮件”）
2. 【中策】：（针对未来一周的策略布局）
3. 【缓策】：（针对大方向的长期避坑或蓄力建议）

## ⚠️ 避坑指南
（单独点出最危险的一步，例如：“最忌酉日往东南方谈判，否则用卦兑金克体，前功尽弃”）

---

# 🚫 语言红线（违者无效）
- 禁止使用“可能”、“大概”、“也许”、“似乎”等模棱两可的词，改用“宜”、“忌”、“必”、“当止”。
- 禁止脱离 {question} 空谈五行理论，每句话必须落回用户的具体场景。
- 若八字信息不全，绝不可编造“日主强弱”等数据，只能依卦独断。

现在，请按以上全流程展开推演，输出你最终的裁决。
"""
